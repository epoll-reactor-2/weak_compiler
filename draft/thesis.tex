...
void f() {
	...
	\section{Optymalizacje kodu pośredniego}
		...
		\subsection{Unreachable code elimination}
			...
			\textbf{Definicja}
			\\
			Niech $G = (V, E)$ -- graf skierowany. Niech $i$ -- wierzchołek startowy grafu $G$.
			$G' = (V', E') \subseteq G$ -- podgraf, gdzie każdy wierzchołek $v \in G'$ jest nieosiągalny z
			dowolnej ścieżki $(i, ...,  v) \in G$. Zauważmy, że $G'$ może być grafem rozłącznym.

			$$i = initial(G), \forall v \in G' \ \nexists \ path (i, ..., v) $$
