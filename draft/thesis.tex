...
%%
	...
	\section{Analiza składniowa}
		...
		\subsection{Definicja}
			...
			drzewiastej, a mianowicie AST (Abstract Syntax Tree). Drzewo syntaksyczne może wyglądać w
			następujący sposób:
			\\
    ...
    int32_t value;
		...
		\subsection{Implementacja analizatora składniowego}
	  	...
	  	\subsection{Reprezentacja wizualna AST}
			...
			\begin{lstlisting}[caption={Tekstowa reprezentacja AST}, label={lst:ast-text}]
...
%			syntaksycznego.
		...
		\section{System typów}
			...
			Dodamy do $\oplus$ operacje tylko dla $\mathbb{N}: \oplus \ \cup \ \{ |, \&, \string^, <<, >>, \% \}$, wtedy
				...
				\inferrule
				  {\Gamma \vdash e_1 : int \\ \Gamma \vdash e_2 : char}
				  {\Gamma \vdash e_1 \oplus e_2 : char}
				\end{mathpar}
				
				\spacing
				
				Wprowadzimy reguły niejawnej konwersji, które są niezbędne przy sprawdzaniu w warunku
