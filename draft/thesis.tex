		...
		\begin{center}
			\begin{tabularx}{\textwidth}{ L{9cm} L{9cm} }
				...
				\begin{tikzpicture}[x=1.05cm,y=0.7cm]
					...
					\separator
					\cell{\texttt{call main()}}     \cellcomL{global=0}          \coordinate (p0)    at (currentcell.east);
					\cell{\texttt{int a = 0}}       \cellcomL{global=4}          \coordinate (p4)    at (currentcell.east);
					\cell{\texttt{int b = 0}}       \cellcomL{global=8}          \coordinate (p8n1)  at (currentcell.east);
					\cell{\texttt{call f()}}        \cellcomL{global=8}          \coordinate (p8n2)  at (currentcell.east);
					\cell{\texttt{int c = 0}}       \cellcomL{global=12,local=4} \coordinate (p12)   at (currentcell.east);
					\cell{\texttt{int d = 0}}       \cellcomL{global=16,local=8} \coordinate (p16n1) at (currentcell.east);
					\cell{\texttt{return from f()}} \cellcomL{global=8}          \coordinate (p16n2) at (currentcell.east);
					\cell{\texttt{return b}}        \cellcomL{global=8}          \coordinate (p16n3) at (currentcell.east);
					\stackbottom{}
					\cell[draw=none]{Stos}

					% todo: Draw from borders.
					\draw[->] (p16n3) to [bend right=100] (p8n1);
					\draw[->] (p8n1)  to [bend left=100]  (p16n1);
				\end{tikzpicture}
				&
				...
				$$$$
				Dany interpreter działa używając stosu. Jest to struktura danych, pozwalająca dodawać i 
